...
import controller.ProductControllerSqlite;
...
public class AdminServlet extends HttpServlet {
	ProductController controller = ProductControllerSqlite.getImpl();
	//change this
	//ProductController controller = ProductControllerImpl.getImplementation();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (checkValue(name) && checkValue(link) && checkValue(description) && checkValue(cost) && isAdmin(req.getSession())) {
			int costInt = 0;
			try {
			...
			controller.addProduct(new Product(name, description, link, costInt));
		}
		allProduct = controller.getProducts();


		if (isAdmin(req.getSession())) {
			Document doc = Jsoup.parse(HtmlReader.getHeder());
		...
		resp.getWriter().println(HtmlReader.heder);
		resp.getWriter().println();
		resp.getWriter().append("<div class=\"container\"><h2>Добавление товара</h2><form action=\"admin\" method=\"post\">\n" + "  <div class=\"form-group\">\n"
		...
		for (int i = 0; i < allProduct.size(); i++) {
			resp.getWriter()
					.append("    <tr>\n" + "      <th scope=\"row\">" + allProduct.get(i).getName() + "</th>\n"
							+ "      <td><img src=\"" + allProduct.get(i).getImgLink() + "\" width=\"200\" height=\"150\" ></td>\n" + "      <td>"
							+ allProduct.get(i).getDescription() + "</td>\n"
							+ "      <td><button type=\"button\" href=\"#\" class=\"btn btn-danger\"  onclick=\" var xhr = new XMLHttpRequest();xhr.open('DELETE', 'admin/"
							+ allProduct.get(i).getId() + "', false);xhr.send();window.location.href='admin'\">Удалить</button></td></tr>");
	...
	protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (isAdmin(req.getSession())) {
			...
			List<Product> productForRemuve = allProduct.stream()
					.filter(product -> product.getId() == id)
					.collect(Collectors.toList());
			controller.removeProduct(productForRemuve.get(0));
